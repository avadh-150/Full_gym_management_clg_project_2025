...
<div id="content">
  ...
  <div class="container-fluid">
    ...
    <div class="row-fluid">
      ...
      if(isset($_REQUEST['search_products']))
      {
        $search=$_POST['search_products'];

                            // Fetch products from the database
                            $result = $con->query("SELECT p.id, p.name AS product_name, p.price, c.name AS category_name, p.status, p.quantity, p.description, p.image
                                           FROM products p
                                           JOIN product_categories c ON p.category_id = c.id
                                           WHERE p.status = 1 AND product_name LIKE '%$search%'");
                            ...
                            <tbody>";
                                ...
                                while ($product = $result->fetch_assoc()) {
                                    ...
                                    <td><div class='text-center'>" . substr($product['description'], 0, 30) . '...' . "</div></td>
                                    ...
                                    <td><div class='text-center'><a href='actions/delete-member.php?pro_id=" . $product['id'] . "' style='color:#F66;'><i class='fa fa-trash' aria-hidden='true'></i> Remove</a></div><br>
                                    <div class='text-center'><a href='edit-product.php?id=" . $product['id'] . "'><i class='fas fa-edit'></i> Edit</a></div>
                                    </tr>
</td>
                                  ";
